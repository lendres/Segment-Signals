...
namespace UnitTests
{
	...
	TEST_CLASS(SegmentSignalFunctionsUnitTest)
	{
	private:
		...
		// For debugging information only.  Used to check that the data file is located correctly relative to current working directory.
		char				_path[FILENAME_MAX];

	public:
		// Default constructor.
		SegmentSignalFunctionsUnitTest()
		{
		...
		TEST_METHOD(MovingAverageTest)
		{
			...
			int    order = 10/2;
			MovingAverage(_data[(int)InputData::Log], _numberOfDataPoints, order, output);
		...
		TEST_METHOD(CalculateMeanAndVarianceTest)
		{
			...
			double mean;
			double variance;
			CalculateMeanAndVariance(_data[(int)InputData::Log], _numberOfDataPoints, 0, mean, variance);
		...
		TEST_METHOD(LogSeparateTest)
		{
			...
			SegmentSignal(_data[(int)InputData::Log], 100, f, order, order1, rmode, niter, Q, numberOfBinaryEvents, FLTLOG, SEGLOG, R, C, D, NACT, error);
			...
			for (int i = 0; i < _numberOfDataPoints; i++)
			{
				Assert::AreEqual(_data[(int)InputData::SegmentedLog][i], SEGLOG[i], 0.1, L"Segmented log failed.");
				Assert::AreEqual(_data[(int)InputData::EventSequence][i], Q[i], 0.1, L"Event sequence failed.");
				Assert::AreEqual(_data[(int)InputData::FilteredLog][i], FLTLOG[i], 0.3, L"Filter log failed.");
			}

			// Write a file for inspection and plotting.
			WriteResultsFile(_data[(int)InputData::Log], SEGLOG, Q, FLTLOG);
		}
